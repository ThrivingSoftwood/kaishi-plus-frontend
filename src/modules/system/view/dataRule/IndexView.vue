<template>
  <div class="app-container">
      ...
      <template #header>
        <div class="card-header">
          ...
          <div style="display: flex; gap: 10px;">
            <el-input v-model="searchResource" clearable placeholder="搜索目标表/资源"
                      prefix-icon="Search" style="width: 180px;"/>
            <el-input v-model="searchName" clearable placeholder="搜索规则名称" prefix-icon="Search"
                      style="width: 180px;"/>
            <el-button icon="Plus" type="primary" @click="openDialog(null)">新建规则</el-button>
          </div>
        </div>
      </template>

      <!-- 🌟 将 :data="ruleList" 改为 :data="filteredRuleList" -->
      <el-table v-loading="loading" :data="filteredRuleList" border height="calc(100vh - 200px)"
                stripe>
        <el-table-column align="center" label="序号" type="index" width="60"/>
        <el-table-column label="受控目标表/资源" prop="targetResource" width="180">
        ...
        <el-table-column align="center" label="策略类型" prop="scopeType" width="160">
          <template #default="{ row }">
            ...
            <el-tag v-else-if="row.scopeType===4" effect="dark" type="warning">本部门及下属</el-tag>
            <el-tag v-else-if="row.scopeType===5" type="danger">高级自定义条件</el-tag>
          </template>
        </el-table-column>

        <el-table-column label="底层 SQL JSON" min-width="250" prop="customSqlJson"
    ...
    <!-- 2. 🌟 核心：可视化规则配置弹窗 -->
    <el-dialog v-model="dialogVisible" :title="dialogTitle" destroy-on-close width="680px">
      <el-form ref="ruleFormRef" :model="ruleForm" :rules="ruleRules" label-width="100px">
        ...
        <el-form-item label="规则类型" prop="scopeType">
          <el-select v-model="ruleForm.scopeType" style="width: 100%">
            <el-option :value="1" label="1 - 全部数据 (不拦截)"/>
            <el-option :value="2" label="2 - 仅看本人数据"/>
            <el-option :value="3" label="3 - 仅看本部门数据"/>
            <el-option :value="4" label="4 - 本部门及下属"/>
            <el-option :value="5" label="5 - ⚙️ 高级自定义条件 (可视化生成)"/>
          </el-select>
        </el-form-item>
        ...
        <template v-if="ruleForm.scopeType === 5">
          ...
          <div v-for="(cond, index) in conditions" :key="index" class="condition-row">
            <!-- 列名 -->
            <el-input v-model="cond.column" placeholder="表别名.列名 (如 a.ktypeid)"
                      style="width: 180px"/>
            ...
            <!-- 删除条件 -->
            <el-button icon="Delete" link style="margin-left: 8px" type="danger"
                       @click="removeCondition(index)"/>
          </div>

          <el-button icon="Plus" plain style="margin-top: 10px; width: 100%" type="primary"
                     @click="addCondition">
            添加查询条件
          </el-button>
        </template>
      ...
      <template #footer>
        ...
        <el-button :loading="submitLoading" type="primary" @click="submitForm">确定保存</el-button>
      </template>
    </el-dialog>
  </div>
</template>
...
import {deleteDataRuleApi, getAllDataRulesApi, saveDataRuleApi} from '@/modules/system/api/dataRule'

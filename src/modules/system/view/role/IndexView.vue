<template>
  <div class="app-container">
      ...
      <template #header>
        <div class="card-header">
          ...
          <div style="display: flex; gap: 10px;">
            <el-input v-model="searchQuery" clearable placeholder="搜索角色名称"
                      prefix-icon="Search" style="width: 200px;"/>
            <el-button icon="Plus" type="primary" @click="openRoleDialog(null)">新增角色</el-button>
          </div>
        </div>
      </template>

      <!-- 🌟 将 :data="roleList" 改为 :data="filteredRoleList" -->
      <el-table v-loading="loading" :data="filteredRoleList" border height="calc(100vh - 200px)"
                stripe>
        <el-table-column align="center" label="序号" type="index" width="60"/>
        <el-table-column label="角色名称" min-width="150" prop="roleName"/>
        <el-table-column label="角色编码" min-width="150" prop="roleCode">
        ...
        <!-- 🌟 核心引擎：三大分配入口 -->
        <el-table-column align="center" label="权限装配 (核心流转)" min-width="320">
          <template #default="{ row }">
            <div class="action-buttons">
              <!-- 绑定物理人 -->
              <el-button icon="User" plain size="small" type="primary"
                         @click="openAssignUsers(row)">
                分配用户
              </el-button>
              <!-- 绑定功能与字段 -->
              <el-button icon="Menu" plain size="small" type="success"
                         @click="openAssignPerms(row)">
                功能与字段
              </el-button>
              <!-- 🌟 绑定行级过滤规则 -->
              <el-button icon="Filter" plain size="small" type="warning"
                         @click="openAssignRules(row)">
                数据范围
              </el-button>
            </div>
          </template>
        ...
        <el-table-column align="center" fixed="right" label="操作" width="160">
          <template #default="{ row }">
            <el-button icon="Edit" link type="primary" @click="openRoleDialog(row)">编辑</el-button>
            <el-button icon="Delete" link type="danger" @click="handleDeleteRole(row)">删除
            </el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <!-- 🌟 2. 角色基础信息弹窗 -->
    <el-dialog v-model="dialogVisible" :title="dialogTitle" destroy-on-close width="500px">
      <el-form ref="roleFormRef" :model="roleForm" :rules="roleRules" label-width="100px">
        ...
        <el-form-item label="角色编码" prop="roleCode">
          <el-input v-model.trim="roleForm.roleCode"
                    ...
                    placeholder="请输入唯一编码(暂不支持中文)，如: BUYER_EAST"/>
        </el-form-item>
      ...
      <template #footer>
        ...
        <el-button :loading="submitLoading" type="primary" @click="submitRole">确定保存</el-button>
      </template>
    </el-dialog>

    <!-- 🌟 3. 抽屉 A：分配用户 (基于部门混合树) -->
    <el-drawer v-model="drawerUserVisible" :title="`为【${currentRow?.roleName}】分配用户`"
               destroy-on-close size="450px">
      <div v-loading="drawerLoading" class="drawer-content">
        <el-alert show-icon style="margin-bottom: 15px;" title="勾选部门可快捷全选该部门下的所有员工"
                  type="info"/>

        <!-- 🌟 新增搜索框 -->
        <el-input v-model="userSearchQuery" clearable placeholder="输入名称或账号快捷筛选"
        ...
        >
          <template #default="{ node, data }">
            <span class="custom-tree-node">
              ...
              <span :class="{ 'is-dept': data.nodeType === 1 }"
                    style="margin-left: 6px;">{{ node.label }}</span>
      ...
      <template #footer>
        ...
        <el-button :loading="submitLoading" type="primary" @click="submitAssignUsers">保存用户分配
        </el-button>
      </template>
    </el-drawer>

    <!-- 🌟 4. 抽屉 B：分配功能权限 (RBAC) -->
    <el-drawer v-model="drawerPermVisible" :title="`为【${currentRow?.roleName}】分配功能与字段`"
               destroy-on-close size="450px">
      <div v-loading="drawerLoading" class="drawer-content">
        <el-alert show-icon style="margin-bottom: 15px;" title="配置立即生效，相关用户将在后台无感刷新权限"
                  type="success"/>
        <el-tree
          ref="permTreeRef"
          :data="permTreeData"
        ...
        >
          <template #default="{ node, data }">
            <span class="custom-tree-node">
              <el-tag v-if="data.permissionType === 1" class="mr-2" size="small"
                      type="info">目录</el-tag>
              <el-tag v-else-if="data.permissionType === 2" class="mr-2" size="small"
                      type="success">菜单</el-tag>
              <el-tag v-else-if="data.permissionType === 3" class="mr-2" size="small"
                      type="warning">按钮</el-tag>
              <el-tag v-else-if="data.permissionType === 5" class="mr-2" effect="dark" size="small"
                      type="danger">字段</el-tag>
              <span :class="{ 'is-dept': data.permissionType === 1 || data.permissionType === 2 }">{{
                  node.label
                }}</span>
            </span>
          </template>
      ...
      <template #footer>
        ...
        <el-button :loading="submitLoading" type="primary" @click="submitAssignPerms">保存功能权限
        </el-button>
      </template>
    </el-drawer>

    <!-- 🌟 5. 抽屉 C：分配数据规则 (ABAC) -->
    <el-drawer v-model="drawerRuleVisible" :title="`为【${currentRow?.roleName}】配置数据范围`"
               destroy-on-close size="600px">
      <div v-loading="drawerLoading" class="drawer-content">
        <el-alert show-icon style="margin-bottom: 15px;"
                  title="勾选后，该角色查询业务表时将被强制加上底层 SQL 拦截" type="warning"/>
        ...
        <el-table
          ...
          :data="allRulesData"
          border
          stripe @selection-change="handleRuleSelection"
        >
          <el-table-column align="center" type="selection" width="50"/>
          <el-table-column label="受控资源" prop="targetResource" width="130">
            <template #default="{ row }">
              <el-tag size="small">{{ row.targetResource }}</el-tag>
            </template>
          </el-table-column>
          <el-table-column label="规则名称" min-width="150" prop="ruleName" show-overflow-tooltip/>
          <el-table-column align="center" label="规则类型" prop="scopeType" width="130">
            <template #default="{ row }">
              <el-tag v-if="row.scopeType===1" size="small" type="success">全部数据</el-tag>
              <el-tag v-else-if="row.scopeType===2" size="small" type="info">仅看本人</el-tag>
              <el-tag v-else-if="row.scopeType===3" size="small" type="warning">本部门</el-tag>
              <el-tag v-else-if="row.scopeType===4" effect="dark" size="small" type="warning">
                本部门及下属
              </el-tag>
              <el-tag v-else-if="row.scopeType===5" size="small" type="danger">自定义</el-tag>
            </template>
          </el-table-column>
        </el-table>
      </div>
      <template #footer>
        ...
        <el-button :loading="submitLoading" type="primary" @click="submitAssignRules">保存规则配置
        </el-button>
      </template>
    </el-drawer>
...
import type {ElTable, ElTree, FormInstance, FormRules} from 'element-plus'
...
import {getSystemUserTreeApi} from '@/modules/system/api/user'
import {getPermissionTreeApi} from '@/modules/system/api/permission'
import {getAllDataRulesApi} from '@/modules/system/api/dataRule' // 🌟 新增 API 引入

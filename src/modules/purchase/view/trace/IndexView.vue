<!-- src/view/purchase/trace/IndexView.vue -->
<template>
  <div class="trace-container">
    ...
    <el-card class="filter-card" shadow="never">
      <el-form :inline="true" :model="currentParams" size="default" @submit.prevent="handleQuery">
        <el-form-item label="单据号">
          <el-input v-model.number="currentParams!.vchcode" clearable placeholder="精准查询单据号"/>
        </el-form-item>
        <el-form-item label="详单号">
          <el-input v-model.number="currentParams!.dlyorder" clearable
                    placeholder="精准查询详单号"/>
        </el-form-item>
        <el-form-item label="订单号">
          <el-input v-model.trim="currentParams!.number" clearable placeholder="模糊搜索订单号"/>
        </el-form-item>
    ...
    <el-card class="table-card" shadow="never">
      <div class="table-wrapper">
        <el-table v-loading="!!currentLoading" :data="currentList" border height="100%" stripe
                  style="width: 100%">
          <!-- 2.1 固定列：序号 -->
          ...
          <!-- 2.3 配置驱动：统计组 (多级表头) -->
          <el-table-column align="center" label="数量与金额统计">
            <template v-for="sub in statsColumns" :key="sub.prop">
              ...
              >
                <template #default="{ row }">
                  ...
                  <span v-else-if="sub.prop === 'stockedQty'" class="text-success">{{
                      row.stockedQty
                    }}</span>
                  <span v-else-if="sub.prop === 'owedQty'"
                        :class="row.owedQty > 0 ? 'text-danger' : 'text-gray'">
                    {{ row.owedQty }}
                  </span>
          ...
          <el-table-column align="center" label="状态" prop="statusName" width="100">
            <template #default="{ row }">
              ...
              <el-tag v-if="row.redWord === '是'" effect="plain" size="small" style="margin-top: 4px"
                      type="danger">红冲
              </el-tag>
            </template>
          </el-table-column>
          ...
          <!-- 2.5 辅助信息 (不走配置) -->
          <el-table-column label="单据摘要" min-width="200" prop="summary" show-overflow-tooltip/>
          <el-table-column label="备注" min-width="150" prop="comment" show-overflow-tooltip/>

          <!-- 2.6 操作列 -->
          <el-table-column align="center" fixed="right" label="操作" width="100">
            <template #default="{ row }">
              <el-button icon="View" link type="primary" @click="goToDetail(row)">详情</el-button>
            </template>
          </el-table-column>
        </el-table>
      </div>

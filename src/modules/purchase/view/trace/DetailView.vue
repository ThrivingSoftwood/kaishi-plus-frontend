...
<template>
  <div class="detail-container">
      ...
      <el-page-header title="返回列表" @back="goBack">
        ...
        <template #extra>
          <el-button
            ...
            :icon="Check" type="success" @click="handleManualFinish"
          >置为完成
          </el-button>
        </template>
      </el-page-header>
    ...
    <!-- 2. 采购单概览信息：配置驱动 -->
    <el-card v-loading="traceStore.isDetailLoading" class="info-card" shadow="never">
      <template #header>
      ...
      <el-empty v-else :image-size="100" description="暂无采购单主信息"/>
    </el-card>

    <!-- 3. 入库实绩表格：配置驱动 -->
    <el-card class="table-card" shadow="never">
      <template #header>
        <div class="card-header"><span>历史入库实绩</span></div>
      </template>
      <el-table v-loading="traceStore.isDetailLoading" :data="traceStore.detailList" border height="calc(100vh - 620px)"
                stripe>
